 ...
 * `skillId` injected. Two exceptions:
 *   - skillId 'audit' triggers the specialized auditConfig (custom hooks,
 *     content blocks, screens).
 *   - skillId 'migrate-*' triggers migrationConfig so each vendor sub uses
 *     the migration steps, content deck, and abort cases. The skillId on the
 *     returned config lands on session.skillId, which migrationConfig.run
 *     (a function) reads to pick the right vendor skill.
 */
function configForCliEntry(entry: CliEntry): ProgramConfig {
  if (entry.skillId === 'audit') return auditConfig;
  if (entry.skillId.startsWith('migrate-')) {
    return { ...migrationConfig, skillId: entry.skillId };
  }
