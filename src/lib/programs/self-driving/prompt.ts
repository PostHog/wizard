...
STEP 2 — Read project and current Signals state. (skill: "Read context")
   Work from the project-state block above and the skill's server-side
   usage probes, then do a light repo scan for what neither covers — repo
   evidence rules products in, never out. List the currently enabled signal
   sources so every later write is idempotent.
