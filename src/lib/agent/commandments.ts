...
// Only sessions that register the TaskCreate/TaskUpdate todo tool (the linear
// pipeline, both harnesses) should be told to drive it. Orchestrator task-mode
// sessions (`pi/task.ts`) never register that tool — TASK_BASICS already gives
// them their own completion contract (`complete_task`) — so including this
// commandment there told agents to use a tool that does not exist.
const TASK_TRACKING_COMMANDMENT =
...
function wizardCommandments(opts?: { taskTracking?: boolean }): string[] {
  return [
    ...
    ...(opts?.taskTracking === false ? [] : [TASK_TRACKING_COMMANDMENT]),
...
export function getWizardCommandments(opts?: {
  taskTracking?: boolean;
}): string {
  return wizardCommandments(opts).join('\n');
}
