...
/**
 * What the agent reads of a program's run definition.
 *
 * A program's `ProgramRun` (`src/lib/programs/program-run.ts`) extends this
 * with hooks that take the wizard session; the caller binds those and hands
 * the agent `RunConfig.hooks` instead. Everything below is plain data or a
 * pure function of agent-side context.
 */
...
export interface RunConfig {
  ...
  programId: string;
  /** The program's run definition, minus the session-bound hooks. */
