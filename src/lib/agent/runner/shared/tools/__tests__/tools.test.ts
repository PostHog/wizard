...
import { BUILTIN_TOOLS, toToolSpecs, createToolDispatcher } from '../registry';
...
describe('registry', () => {
  ...
  it('routes a tool call to its handler', async () => {
  ...
  it('returns an error result for an unknown tool', async () => {
    const dispatcher = createToolDispatcher(ctx);
    const res = await dispatcher.dispatch({ name: 'Nonexistent', input: {} });
    expect(res.isError).toBe(true);
