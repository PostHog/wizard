...
export function createWizardPiTaskTools(): {
  ...
  const taskCreate = defineTool({
    name: 'TaskCreate',
    label: 'Create task',
    description: 'Create a task in the shared todo list. Returns its assigned id.',
    promptSnippet: 'TaskCreate(content) — add a todo (surfaces progress in the UI)',
  ...
  const taskUpdate = defineTool({
    name: 'TaskUpdate',
    label: 'Update task',
    description: 'Update an existing task by id (status, content, or activeForm).',
    promptSnippet: 'TaskUpdate(taskId, status) — mark a todo in_progress/completed',
    parameters: Type.Object({
      taskId: Type.String(),
  ...
  const taskGet = defineTool({
    ...
    async execute(_id, args) {
      ...
      return text(
        t ? JSON.stringify({ id: args.taskId, ...t }) : `No such task: ${args.taskId}`,
      );
    },
  });
  ...
  const taskList = defineTool({
    ...
    async execute() {
      return text(
        JSON.stringify(Array.from(store.entries()).map(([id, t]) => ({ id, ...t }))),
      );
    },
  });
