...
import { scan, type HookPhase, type ToolTarget } from '../../../yara-scanner';
import { isWizardDocumentationPath } from '../../../yara-hooks';
  ...
  const factory = (pi: PiExtensionApiLike): void => {
    pi.on('tool_call', (event) => {
      ...
      if (state.criticalViolation) {
        return { block: true, reason: 'Run terminated by a security violation.' };
      }
      state.toolCalls += 1;
