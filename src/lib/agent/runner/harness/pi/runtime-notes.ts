...
const BASH_SCOPE = [
  ...
  '- For Python, install into a virtual environment, never the system interpreter (which is often externally managed and rejects a direct `pip install`). FIRST look for one that already exists — `ls`/`find` for `.venv/`, `venv/`, or `env/` in the project root, and check whether the project is tool-managed (a `poetry.lock`, `uv.lock`, or `Pipfile.lock` means you install with `poetry add` / `uv add` / `pipenv install` and never touch a venv yourself). Create one ONLY if that search finds nothing: `python -m venv .venv`. Either way, install through that interpreter explicitly — `.venv/bin/pip install …`, `.venv/bin/python …` — and never bare `pip`, which is usually absent from PATH.',
];
