...
describe('pi-security: blocked-action corpus (parity with the anthropic fence)', () => {
  ...
  test('allows a framework’s declared package managers, denies others', async () => {
    // A ctx carrying the active framework's allowedPackageManagers threads
    // straight through evaluateToolCall into wizardCanUseTool.
    const withManagers = async (
      command: string,
      allowedPackageManagers: readonly string[],
    ) =>
      (await evaluateToolCall('bash', { command }, { allowedPackageManagers }))
        .block;

    // PHP / Laravel
    expect(
      await withManagers('composer require posthog/posthog-php', ['composer']),
    ).toBe(false);
    // Ruby / Rails
    expect(await withManagers('bundle add posthog-ruby', ['bundle'])).toBe(
      false,
    );
    expect(await withManagers('bundle install', ['bundle'])).toBe(false);
    expect(await withManagers('gem install posthog-ruby', ['gem'])).toBe(false);
    // Android / JVM
    expect(await withManagers('./gradlew assembleDebug', ['./gradlew'])).toBe(
      false,
    );
    expect(await withManagers('mvn install', ['mvn'])).toBe(false);
    // iOS
    expect(
      await withManagers(
        'swift package add-dependency https://github.com/PostHog/posthog-ios',
        ['swift'],
      ),
    ).toBe(false);
    expect(await withManagers('pod install', ['pod'])).toBe(false);
    // Go / Rust / .NET / Flutter / Elixir / extra Python
    expect(
      await withManagers('go get github.com/posthog/posthog-go', ['go']),
    ).toBe(false);
    expect(await withManagers('cargo add posthog-rs', ['cargo'])).toBe(false);
    expect(await withManagers('dotnet add package PostHog', ['dotnet'])).toBe(
      false,
    );
    expect(
      await withManagers('flutter pub add posthog_flutter', ['flutter']),
    ).toBe(false);
    expect(await withManagers('mix deps.get', ['mix'])).toBe(false);
    expect(await withManagers('pdm add posthog', ['pdm'])).toBe(false);

    // A manager the framework did NOT declare is still denied.
    expect(await withManagers('composer require x', ['npm'])).toBe(true);

    // Arbitrary code execution stays blocked even for an allowed binary,
    // because the fence only permits install/build verbs (run/exec skipped).
    expect(await withManagers('go run main.go', ['go'])).toBe(true);
    expect(await withManagers('swift run', ['swift'])).toBe(true);
    expect(await withManagers('cargo run', ['cargo'])).toBe(true);
  });

  test('without a framework list, only the JS+Python default is allowed', async () => {
    // No allowedPackageManagers in ctx -> DEFAULT_PACKAGE_MANAGERS applies.
    expect(await block('bash', { command: 'pip install posthog' })).toBe(false);
    expect(await block('bash', { command: 'uv add posthog' })).toBe(false);
    expect(
