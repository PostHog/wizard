...
describe('pi-security extension: read-before-write parity (claude-agent-sdk semantics)', () => {
  ...
  let dir: string;
  beforeEach(() => {
    dir = fs.mkdtempSync(path.join(os.tmpdir(), 'wizard-pi-rbw-'));
  });
  afterEach(() => {
    fs.rmSync(dir, { recursive: true, force: true });
  });

  test('write/edit of an unread existing file blocks; a read unlocks it', async () => {
    fs.writeFileSync(path.join(dir, 'existing.ts'), 'export {};');
    const { factory } = createSecurityExtension({ workingDirectory: dir });
    const { pi, handlers } = fakePi();
    factory(pi);
    ...
    expect(
      await handlers.tool_call({
        toolName: 'write',
        input: { path: 'existing.ts', content: 'export const x = 1;' },
      }),
    ).toEqual({
      block: true,
      reason: expect.stringMatching(/File has not been read yet/),
    });

    // The agent reads it → the same write is now allowed.
    await handlers.tool_result({
      toolName: 'read',
  ...
  test('brand-new files need no read, and an own write unlocks later edits', async () => {
    const { factory } = createSecurityExtension({ workingDirectory: dir });
    const { pi, handlers } = fakePi();
    factory(pi);
    ...
    // The tool executed: the file exists now, its result is recorded.
    fs.writeFileSync(path.join(dir, 'new.ts'), 'export {};');
    await handlers.tool_result({
      toolName: 'write',
      input: { path: 'new.ts' },
      content: [{ type: 'text', text: 'Successfully wrote 11 bytes' }],
      isError: false,
    });
    ...
    expect(
      await handlers.tool_call({
        ...
        input: {
          path: 'new.ts',
          edits: [{ oldText: 'export {};', newText: 'export const y = 2;' }],
        },
      }),
    ).toEqual({});
  });

  test('a failed read does not unlock the file', async () => {
    fs.writeFileSync(path.join(dir, 'existing.ts'), 'export {};');
    const { factory } = createSecurityExtension({ workingDirectory: dir });
    const { pi, handlers } = fakePi();
    factory(pi);
