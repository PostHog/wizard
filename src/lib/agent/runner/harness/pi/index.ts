...
const PI_RUNTIME_NOTES = [
  ...
  '- Use the Task tools to plan and track the whole run so the user always sees where you are. Create the task list once you understand the work — after you load and skim the skill workflow, not before — and cover every stage through to instrumenting events, creating the dashboard, and writing the setup report. Keep it current: the moment you find work the plan is missing, add a task for it.',
  '- Keep exactly ONE task `in_progress`. `TaskUpdate` it to `in_progress` right before you start that stage, and to `completed` the instant you finish it — mark completions one at a time as they happen, never batched at the end. Only mark `completed` when the work is genuinely done; if a stage is blocked, leave it `in_progress` and add a task for the fix.',
  `- Status updates are PLAIN TEXT you write in your reply, NOT a tool call — there is no status tool. Whenever you begin a new action, write a line that starts with the literal marker ${AgentSignals.STATUS} followed by a short present-tense phrase (e.g. "${AgentSignals.STATUS} Reading the router entry", "${AgentSignals.STATUS} Adding the login capture"). The harness automatically parses any line beginning with ${AgentSignals.STATUS} and shows it as the live status. Do this OFTEN — several times per task, on every meaningful shift, not once per phase. It is free.`,
