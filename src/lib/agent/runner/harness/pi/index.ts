...
const PI_RUNTIME_NOTES = [
  ...
  '- Drive the list strictly top to bottom: keep exactly ONE task `in_progress`, finish it, `TaskUpdate` it to `completed`, then start the next. Mark every transition the MOMENT it happens — never leave a finished task unmarked, and never mark a later task done while earlier ones are still pending.',
  '- If you end up doing the work in a different order than listed, call `TaskReorder` with the task ids in the order you are actually doing them, so the list always reads top-to-bottom in real execution order.',
