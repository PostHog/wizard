 ...
 * time. Steering it once up front avoids the retry spirals. Also carries the
 * autonomous-run discipline the anthropic harness inherits from its preset
 * system prompt: don't stall by narrating instead of acting, don't mark work
 * done that isn't, and don't let the setup report claim more than was verified.
 */
const PI_RUNTIME_NOTES = [
  ...
  '- Try to keep exactly ONE task `in_progress`. `TaskUpdate` it to `in_progress` right before you start that stage, and to `completed` the instant you finish it — one at a time, never batched at the end. Never mark a task `completed` while the build is failing, the implementation is partial, an error is unresolved, or a file or dependency you needed could not be found — keep it `in_progress` and add a task for the fix.',
  ...
  '- You are operating autonomously: nobody is watching live, and plain text you write cannot be answered. Before ending a turn, check what you just wrote — if it is a plan, a question, or a promise about work not yet done ("next I\'ll add the provider"), do that work NOW with a tool call in the same turn, because a turn without a tool call ends the run with that work undone. Never wind down because the run feels long; the run is over when every task is completed and the setup report is written, not before.',
  "- Complete the integration fully — don't gold-plate, but don't leave it half-done. Installing and initializing the SDK is not the finish line; events instrumented, dashboard created, setup report written is.",
  ...
  '- In the setup report, separate what you VERIFIED from what you could not: a passing build or typecheck proves the code compiles, not that events flow — never write "events are now being captured" unless you observed one arrive. Report outcomes faithfully: if a step failed or was skipped, say so plainly instead of rounding up to success. Anything only the user can confirm belongs in the report\'s "Verify before merging" checklist, each item naming the file (and line) to check.',
  ...
  '- Write code that reads like the surrounding code: match its comment density, naming, and idiom. Never add comments that narrate what a line does or that it was added by the wizard ("// Initialize PostHog", "// Added for analytics") — comment only a constraint the code itself cannot show.',
