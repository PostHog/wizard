...
const PI_RUNTIME_NOTES = [
  ...
  '- Do NOT delete `.posthog-events.json` — `rm` is blocked, and the host removes the file automatically after the run. If a skill step asks you to remove it, skip that removal: update its contents to match the events you actually implemented, leave the file in place, and continue.',
  '- To create a NEW file, call `write` directly — do NOT `read` the path first; reading a missing path just errors and does not count as the pre-edit read. The read-before-edit rule applies to files that already exist: any file you are about to `edit` — including one you created earlier in this run — needs a fresh `read` first.',
