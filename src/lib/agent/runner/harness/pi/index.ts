...
const PI_RUNTIME_NOTES = [
  ...
  '- If you get stuck on something outside your control — a command you are not permitted to run, or a fix outside the scope of this integration — do NOT spiral retrying it. Note it in the setup report for the user to resolve, and move on with the rest of the work.',
  "- A blocked or failing package-manager INSTALL is NOT a reason to skip the integration. Declare the dependency by editing the project's manifest file directly (package.json, composer.json, Gemfile, build.gradle), then complete the full integration — init, identify, captures — exactly as if it were installed, and note the pending install command in the setup report. A diff that adds the dependency plus the instrumentation is correct and complete; the user runs the install. Skipping the instrumentation because the install command was blocked is wrong.",
