...
describe('health-checks', () => {
  ...
  describe('evaluateWizardReadiness', () => {
    ...
    it('proceeds without warnings when Anthropic is degraded', async () => {
      const body = makeStatuspageStatus({
        pageId: 'tymt9n04zgry',
        pageName: 'Claude',
        pageUrl: 'https://status.claude.com',
    ...
    it('proceeds without warnings when MCP is down', async () => {
      (global.fetch as Mock).mockImplementation(
        overrideFetch({
          [URLS.mcpLanding]: () =>
            Promise.resolve(new Response('Bad Gateway', { status: 502 })),
    ...
    it('proceeds without warnings when npm overall is down', async () => {
      const body = makeStatuspageStatus({
        pageId: 'wyvgptkd90hm',
        pageName: 'npm',
        pageUrl: 'https://status.npmjs.org',
    ...
    it('proceeds without warnings when a non-blocking service is degraded', async () => {
      const body = makeStatuspageStatus({
        pageId: 'yh6f0r4529hb',
        pageName: 'Cloudflare',
        pageUrl: 'https://www.cloudflarestatus.com',
