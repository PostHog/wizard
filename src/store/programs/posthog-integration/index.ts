...
import { AgentSignals } from '../../agent-protocol/agent-signals.js';
import {
...
import { WIZARD_INTERACTION_EVENT_NAME } from '../../shared/constants.js';
import { getUI } from '../../ui/index.js';
import { requestDeepLink } from '../../shared/provisioning.js';
import { openTrackedLink, withUtm } from '../../shared/links.js';
...
export const posthogIntegrationConfig: ProgramConfig = {
  ...
  run: async (session: WizardSession): Promise<ProgramRun> => {
    ...
    if (usesPackageJson) {
      ...
      if (packageJson) {
        const { hasDeclaredDependency } = await import(
...
export const integrationRunStep: ProgramStep = {
  ...
  run: { programId: 'posthog-integration' },
  isComplete: (session) =>
