...
export const ANDROID_AGENT_CONFIG: FrameworkConfig<AndroidContext> = {
  ...
  detection: {
    ...
    detect: async (options) => {
      ...
      // A Flutter project ships a full android/ subtree (manifests, gradle
      // wiring, a MainActivity.kt), so both strategies below would claim it
      // and integrate posthog-android into the native shell instead of the
      // Flutter app. pubspec.yaml at the root is the Flutter/Dart marker —
      // bail out so this config never claims a Flutter project.
