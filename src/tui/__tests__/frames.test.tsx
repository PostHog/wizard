...
vi.mock('@store/shared/clipboard', () => ({
  copyToClipboard: vi.fn().mockResolvedValue(true),
  openInBrowser: vi.fn().mockResolvedValue(true),
  browserOpenCommands: vi.fn(() => []),
}));
vi.mock('@store/shared/links', async (actual) => ({
  ...(await actual<Record<string, unknown>>()),
  openTrackedLink: vi.fn(),
}));
vi.mock('@store/shared/debug', async (actual) => ({
  ...(await actual<Record<string, unknown>>()),
  getLogFilePath: () => '/tmp/posthog-wizard.log',
  logToFile: vi.fn(),
  debug: vi.fn(),
}));
vi.mock('@store/shared/setup-utils', async (actual) => ({
  ...(await actual<Record<string, unknown>>()),
  getOrAskForProjectData: vi.fn(pending),
}));
vi.mock('@store/api', async (actual) => ({
  ...(await actual<Record<string, unknown>>()),
...
vi.mock('@store/programs/self-driving/detect-agentic', async (actual) => ({
  ...(await actual<Record<string, unknown>>()),
  detectSelfDrivingIntegrationProjects: vi.fn(pending),
}));
vi.mock('@store/programs/error-tracking/detect-agentic', async (actual) => ({
  ...(await actual<Record<string, unknown>>()),
  detectErrorTrackingProjects: vi.fn(pending),
}));
vi.mock(
  '@store/programs/error-tracking-upload-source-maps/detect-agentic',
  async (actual) => ({
    ...(await actual<Record<string, unknown>>()),
    detectSourceMapsProjects: vi.fn(pending),
  }),
);
vi.mock('@store/programs/posthog-doctor/fetch', () => ({
...
import type { FrameworkConfig } from '@store/framework-config';
...
import { AUDIT_CHECKS_KEY } from '@store/programs/audit/types';
...
import {
  ...
  screenShell,
  type TerminalSize,
} from './helpers/render-screen.no-jest.js';
