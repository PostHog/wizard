...
describe('skillCommandFactory', () => {
  ...
  it('throws when the manifest entry is not a command role', () => {
    expect(() =>
      skillCommandFactory(buildTestEntry({ role: 'skill' }), buildTestConfig()),
    ).toThrow(/role "skill"/);
    expect(() =>
      skillCommandFactory(
        buildTestEntry({ role: 'internal' }),
        buildTestConfig(),
      ),
    ).toThrow(/role "internal"/);
  });
