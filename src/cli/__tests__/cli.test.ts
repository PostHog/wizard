...
vi.mock('@store/programs/posthog-integration', () => ({
...
vi.mock('@store/shared/environment', () => ({
...
vi.mock('@store/registry', () => ({ FRAMEWORK_REGISTRY: {} }));
vi.mock('@store/detection', () => ({
...
describe('CLI argument parsing', () => {
  ...
  describe('local dev flags', () => {
    ...
    test('no global option is named `local`', async () => {
      const { GLOBAL_OPTIONS } = await import('../wizard.js');
  ...
  describe('--ci flag', () => {
    ...
    test("tags the build as 'ci'", async () => {
      ...
      const { analytics } = await import('@store/shared/analytics');
