...
Programs call the agent to do the work a skill describes. The TUI and the headless runner observe the run through `onProgress` and answer it through `interaction`; today `src/lib/runners/run-program-agent.ts` does both on top of the session.
