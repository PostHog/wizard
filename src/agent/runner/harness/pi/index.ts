...
import type { BootstrapResult } from '../../shared/types.js';
import type { TaskStore } from './tasks.js';
import { completionFailure, runErrorType } from './completion.js';
...
export const piBackend: AgentHarness = {
  ...
  async run(inputs: BackendRunInputs): Promise<AgentResult> {
    ...
    try {
      ...
      const { createSecurityExtension } = await import('./security.js');
      ...
      try {
        const { setupPostHogMcp, fetchInstructions } = await import('./mcp.js');
      ...
      const { createWizardPiTools } = await import('./tools.js');
      const { createWizardPiTaskTools } = await import('./tasks.js');
      const { createDispatchAgentTool } = await import('./subagent.js');
  ...
  async runTask(inputs: TaskRunInputs): Promise<AgentResult> {
    const { runPiTask } = await import('./task.js');
