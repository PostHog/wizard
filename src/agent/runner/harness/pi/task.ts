...
import type { AgentResult, TaskRunInputs } from '../types.js';
import {
  gatewayAuth,
  type GatewayAuth,
...
import { runErrorType } from './completion.js';
...
export async function runPiTask(inputs: TaskRunInputs): Promise<AgentResult> {
  ...
  try {
    ...
    if (allowsPostHogMcp(allowedTools)) {
      try {
        const { setupPostHogMcp } = await import('./mcp.js');
    ...
    const { createPiOrchestratorTools } = await import(
