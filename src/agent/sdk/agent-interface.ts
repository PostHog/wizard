...
import { evaluateBashCommand } from '../security/bash-fence';
...
import { scanProjectSkills } from '../security/skill-preflight';
...
import { classifyToolToStage } from '@shared/run/agent-phase';
...
import {
  ...
  REMARK_INSTRUCTION,
  RESUME_INSTRUCTION,
} from '../progress/signals';
import { classifyAuthFailure, WizardError } from '@shared/errors';
import { isGrantRevoked } from '@shared/gateway/auth-session-state';
...
export { AgentSignals, AgentErrorType } from '../progress/signals';
export type { AgentSignal } from '../progress/signals';
export { AgentOutputSignals } from '../progress/output-signals';
import {
...
import { sanitizeAgentSubprocessEnv } from '@shared/claude/agent-env-isolation';
...
export type AgentConfig = {
  ...
  askBridge?: import('../progress/wizard-ask-bridge').WizardAskBridge;
   ...
   */
  getPendingQuestion?: () =>
    | import('../progress/progress').PendingQuestion
    | null;
  /**
...
export { isSkillInstallCommand } from '@shared/skills/skill-install';
    ...
    if (warlockDisabled) {
      ...
      try {
        ...
        if (findings.length > 0) {
          ...
          return {
            error: AgentErrorType.YARA_VIOLATION,
            message:
