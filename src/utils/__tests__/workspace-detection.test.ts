...
// ---------------------------------------------------------------------------
// detectWorkspaces — polyglot supplement
// ---------------------------------------------------------------------------

describe('detectWorkspaces — polyglot supplement', () => {
  it('discovers Python service outside pnpm workspace', async () => {
    await createFile('pnpm-workspace.yaml', `packages:\n  - 'apps/*'\n`);
    await createFile('apps/web/package.json', '{}');
    // Python service NOT in pnpm-workspace.yaml
    await createFile(
    ...
    expect(result!.memberDirs).toHaveLength(2);
    expect(result!.memberDirs).toContain(path.resolve(tmpDir, 'apps/web'));
    expect(result!.memberDirs).toContain(path.resolve(tmpDir, 'services/ml'));
  });

  it('does not duplicate project already in pnpm workspace', async () => {
    await createFile(
      'pnpm-workspace.yaml',
      `packages:\n  - 'apps/*'\n  - common/parser\n`,
    );
    await createFile('apps/web/package.json', '{}');
    await createFile('common/parser/package.json', '{}');
    await createFile(
      'common/parser/pyproject.toml',
      '[project]\nname = "parser"',
    );
  ...
  it('does not add root as supplemental when root is a formal member', async () => {
    await createFile(
      'pnpm-workspace.yaml',
      `packages:\n  - '.'\n  - 'apps/*'\n`,
    );
    ...
    expect(result).not.toBeNull();
    const rootOccurrences = result!.memberDirs.filter(
      (d) => path.resolve(d) === path.resolve(tmpDir),
    );
    expect(rootOccurrences).toHaveLength(1);
  });

  it('discovers non-JS project even when root is a workspace member via dot', async () => {
    // PostHog-like scenario: root '.' is a pnpm member (Django), and a
    // Python service lives outside the workspace config
    await createFile(
      'pnpm-workspace.yaml',
      `packages:\n  - '.'\n  - 'apps/*'\n`,
    );
    ...
    expect(result).not.toBeNull();
    // root (via '.') + apps/web + services/api (supplemental)
    expect(result!.memberDirs).toHaveLength(3);
    expect(result!.memberDirs).toContain(path.resolve(tmpDir, 'services/api'));
  });

  it('does not add subdirectory of existing member', async () => {
    await createFile('pnpm-workspace.yaml', `packages:\n  - 'apps/*'\n`);
    await createFile('apps/backend/package.json', '{}');
    // Nested Python config inside an existing JS member
    await createFile(
      'apps/backend/scripts/pyproject.toml',
      '[project]\nname = "scripts"',
    );
    ...
    expect(result).not.toBeNull();
    expect(result!.memberDirs).toHaveLength(1);
    expect(result!.memberDirs).toContain(path.resolve(tmpDir, 'apps/backend'));
  });

  it('discovers PHP project alongside JS workspace', async () => {
    await createFile('pnpm-workspace.yaml', `packages:\n  - 'frontend/*'\n`);
    await createFile('frontend/web/package.json', '{}');
    await createFile('api/composer.json', '{}');
    ...
    expect(result).not.toBeNull();
    expect(result!.memberDirs).toHaveLength(2);
    expect(result!.memberDirs).toContain(path.resolve(tmpDir, 'api'));
  });

  it('discovers Android project alongside JS workspace', async () => {
    await createFile('pnpm-workspace.yaml', `packages:\n  - 'packages/*'\n`);
    await createFile('packages/shared/package.json', '{}');
    await createFile('android/build.gradle.kts', '');
    ...
    expect(result).not.toBeNull();
    expect(result!.memberDirs).toHaveLength(2);
    expect(result!.memberDirs).toContain(path.resolve(tmpDir, 'android'));
  });

  it('supplements npm workspaces too, not just pnpm', async () => {
    await createFile(
      'package.json',
      JSON.stringify({ workspaces: ['apps/*'] }),
    );
    await createFile('apps/web/package.json', '{}');
    await createFile('services/api/requirements.txt', 'fastapi\n');
  ...
  it('discovers multiple non-JS projects at depth 1 and 2', async () => {
    await createFile('pnpm-workspace.yaml', `packages:\n  - 'apps/*'\n`);
    await createFile('apps/web/package.json', '{}');
    await createFile('backend/requirements.txt', 'django\n');
    await createFile('services/ml/pyproject.toml', '[project]\nname = "ml"');
    await createFile('mobile/build.gradle.kts', '');

    const result = await detectWorkspaces(tmpDir);

    expect(result).not.toBeNull();
    expect(result!.memberDirs).toHaveLength(4);
    expect(result!.memberDirs).toContain(path.resolve(tmpDir, 'backend'));
    expect(result!.memberDirs).toContain(path.resolve(tmpDir, 'services/ml'));
    expect(result!.memberDirs).toContain(path.resolve(tmpDir, 'mobile'));
  });
});
