...
| **`chunk-manifest.no-jest.ts`**    | Prints a structural manifest of `dist/`: per chunk, the source files it contains and the chunks it imports, hash suffixes stripped. `--summary` prints the chunk names and the sorted source set. The Surfaces workflow diffs that summary against `scripts/__fixtures__/chunk-manifest.{prod,ci}.json`. Sourcemap sources differ between macOS and Linux, so record the fixtures from the workflow's `chunk-manifests-<sha>` artifact, never from a local build. | A built `dist/`                                                                                              |
