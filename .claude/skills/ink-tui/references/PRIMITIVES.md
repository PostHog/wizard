...
Pure-render bordered "physical" submit button. Used by the multi-select menus as the final focusable row. Props: `label` (default `"Confirm"`), `focused`, optional `count` (renders `Label (n)`). Accent border + triangle when focused, muted otherwise.
