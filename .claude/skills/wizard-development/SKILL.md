...
| Local tool permissions and scanner adapters           | [agent-interface](../../../src/agent/agent-interface.ts), [YARA hooks](../../../src/store/security/yara-hooks.ts), [Pi security](../../../src/agent/runner/harness/pi/security.ts) |
...
| Screen resolution and rendering                       | [TUI](../../../src/tui/) through [WizardUI](../../../src/store/ui/wizard-ui.ts)                                                                                                    |
...
- **Prefer orchestration.** A seed agent plans work, then task agents execute
  ...
  [metrics](../../../src/store/programs/metrics/) and its context-mill flow.
...
[DEFAULT_BINDING](../../../src/agent/runner/switchboard/index.ts) still selects
Anthropic + linear, with per-program and flag overrides. Set new bindings
explicitly. Migrating an existing program requires checking its flow, tasks, and
lifecycle hooks; changing the default constant alone is insufficient. Both
harnesses implement `run` and `runTask`.

Adding a model or effort is a cross-repository change:

1. Define the Wizard model ID and capabilities in
   [constants](../../../src/store/shared/constants.ts) and
...
[secret-vault](../../../src/store/session/secret-vault.ts) references. Tool
implementations resolve values host-side where they are used; they must not
return the raw value to the model. Inspect the
[wizard-tools](../../../src/store/tools/) implementations and the Pi adapter
