...
Score each judged run from its diff on four dimensions, 1–5 each
(5 production-ready, 3 works with real issues, 1 broken or empty):

- **Files** — right files touched, nothing unrelated, imports valid
- **App** — nothing broken: builds, existing code and configs preserved,
  changes minimal
- **PostHog** — SDK installed, initialized at the right entry points,
  env-based keys, real distinct id, identify, error tracking
- **Events** — real user actions, useful properties, no PII, consistent names
