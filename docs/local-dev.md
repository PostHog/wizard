...
| `--e2e-ask`                | `POSTHOG_WIZARD_E2E_ASK`            | keep `wizard_ask` wired in a `--ci` run so a controlling parent answers the agent's questions    |

`--control-socket <path>` is not a dev flag: every build accepts it with the
experimental headless flag and serves the control API over that unix socket (see
[`e2e-harness/ARCHITECTURE.md`](../e2e-harness/ARCHITECTURE.md)). Dev builds
also accept it with `--ci` to control the real TUI; published TUI runs refuse
it.
